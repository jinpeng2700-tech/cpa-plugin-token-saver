...
func TestReleaseIdentityVerifierRequiresSamePeeledCommit(t *testing.T) {
	...
	runTestCommand(t, repository, "git", "tag", "-a", "v1.1.0", "-m", "release")
	...
	command := exec.Command("sh", script, first, first, metadata, remote, "v1.1.0")
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			command := exec.Command("sh", script, tt.event, tt.build, metadata, remote, "v1.1.0")
			command.Dir = repository
...
func TestReleaseFinalizerEmitsPortableMetadataAndChecksums(t *testing.T) {
	...
	if metadata.Version != "1.1.0" || metadata.Tag != "v1.1.0" ||
...
func runReleaseFinalizer(t *testing.T, dist string, env []string, wantSuccess bool) []byte {
	...
	command := exec.Command("sh", script, dist, "1.1.0", strings.Repeat("a", 40))
	command.Env = env
	output, err := command.CombinedOutput()
