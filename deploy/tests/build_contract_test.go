...
func TestReleaseIdentityVerifierRequiresSamePeeledCommit(t *testing.T) {
	...
	runTestCommand(t, repository, "git", "tag", "-a", "v1.2.1", "-m", "release")
	...
	command := exec.Command("sh", script, first, first, metadata, remote, "v1.2.1")
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			command := exec.Command("sh", script, tt.event, tt.build, metadata, remote, "v1.2.1")
			command.Dir = repository
...
func TestReleaseFinalizerEmitsPortableMetadataAndChecksums(t *testing.T) {
	...
	if metadata.Version != "1.2.1" || metadata.Tag != "v1.2.1" ||
...
func runReleaseFinalizer(t *testing.T, dist string, env []string, wantSuccess bool) []byte {
	...
	command := exec.Command("sh", script, dist, "1.2.1", strings.Repeat("a", 40))
	command.Env = env
	output, err := command.CombinedOutput()
