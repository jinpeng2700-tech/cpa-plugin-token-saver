...
func TestRebuildBundleRoundTripAndSecretRejection(t *testing.T) {
	...
	for name, body := range map[string]string{
		...
		"token-saver-v1.2.6.so": "\x7fELF\x00fake plugin\n",
	...
	args := []string{
		...
		"--input-dir", input,
		"--output-dir", output,
		"--deployment-id", "test-7.2.136-1.2.6",
	...
	if manifest.Plugin.Version != "1.2.6" || manifest.Plugin.ABI != 1 || manifest.Plugin.RPCSchema != 3 ||
